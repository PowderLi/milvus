...
import (
	...
	"github.com/milvus-io/milvus/tests/go_client/compressor/datadog"
...
func TestCompression(t *testing.T) {
	ctx := context.Background()
	for _, compressionName := range []string{"", deflate.Name, lz4.Name, snappy.Name, datadog.Name, gzip.Name} {
...
func Benchmark128MKlauspost(b *testing.B) {
	benchmarkRun(b, zstd.Name, 128, 1024)
}
func Benchmark128MDatadog(b *testing.B) {
	benchmarkRun(b, datadog.Name, 128, 1024)
}
...
func BenchmarkCompressionDatadog(b *testing.B) {
	runGrpcCompressionPerf(b, datadog.Name)
}
